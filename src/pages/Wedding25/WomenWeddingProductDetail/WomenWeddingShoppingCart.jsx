...
export default function WomenWeddingShoppingCart() {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed top-4 right-4 bg-indigo-900 text-white px-6 py-3 rounded-full font-semibold hover:bg-indigo-800 transition-colors shadow-lg z-50"
      >
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-6">
            {cartItems.map((item, index) => (
              <div 
                key={item.id} 
                className={`flex gap-4 py-6 ${index !== cartItems.length - 1 ? 'border-b border-gray-200' : ''}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <h4 className="font-medium text-sm mb-2 leading-tight">{item.name}</h4>
                  <p className="text-sm text-gray-900 mb-3">PKR {item.price.toLocaleString()}</p>
                  ...
                  <div className="space-y-0.5 text-xs text-gray-900">
                    ...
                    <p><span className="font-medium">Actual Price:</span> PKR {item.actualPrice.toLocaleString()}</p>
